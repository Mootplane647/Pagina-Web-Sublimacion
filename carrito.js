...
function allIntegralTotal() {
  ...
  for (singelIntegralElement of allSingelIntegralElement) {
    // Cantidad
    var count =
      singelIntegralElement.parentElement.children[4].children[1].value;
    if (sum != "") {
      sum += "+";
    }
...
// Compra ahora
function buyNow() {
  var result = confirm("¡Compra Exitosa!");
  if (result == false) {
    return;
  }
...
function addToCart(obj) {
  ...
  if (result != "-1") {
    ...
    td2.innerHTML =
      '<img src="' + shop.shopImg + '" height="100" width="100"/>';
    ...
    td7.innerHTML =
      '<a href="#" class="delete" οnclick="deleteChild(this)"> Eliminar </a>';
    tr.appendChild(td1);
    tr.appendChild(td2);
    tr.appendChild(td6);
    // Añadir al carrito
